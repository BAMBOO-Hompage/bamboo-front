...
export default function CreateCurriculum() {
  ...
  const [cohort, setCohort] = useState(6);
  ...
  useEffect(() => {
    ...
    setIsEndActive(month === 2 || month === 8);
  }, [currentDate]);
  ...
  const addSubject = () => {
    setSubjects([
      ...subjects,
      { name: "", weeklyContents: [{ week: "1주차", content: "" }] },
    ]);
  };

  const addWeek = (subjectIndex) => {
    const updatedSubjects = [...subjects];
    updatedSubjects[subjectIndex].weeklyContents.push({
      week: `${updatedSubjects[subjectIndex].weeklyContents.length + 1}주차`,
      content: "",
    });
    setSubjects(updatedSubjects);
  };

  const updateSubjectName = (name, idx) => {
    const updatedSubjects = [...subjects];
    updatedSubjects[idx].name = name;
    setSubjects(updatedSubjects);
  };
